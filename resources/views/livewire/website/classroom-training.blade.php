        ...
        <div class="row">
            <div class="col-auto">
                <div class="breadcrumbs__content">
                    ...
                    <div class="breadcrumbs__item ">
                        <a href="/Classroom-Training-Courses">Classroo Training Courses</a>
                    </div>
            ...
            <div class="row y-gap-30 relative">
                <div class="col-xl-7 col-lg-8">
                    ...
                    <div data-anim="slide-up delay-1">
                        <h1 class="text-30 lh-14 pr-60 lg:pr-0">Classroom Training Courses</h1>
                    </div>
                </div>
                        ...
                        @foreach ($courses_lists as $course)
                            <div class="contact-details-one__single" style="padding: 15px 30px 15px;">
                                <div class="contact-details-one__single-icon">
                                    <span><img decoding="async" data-opt-src="{{ asset('icons/Online-Icon-01-1.png') }}"
                                            ...
                                            alt="{{ $course->Course_Title }}" class="" data-opt-lazy-loaded="true"
                                            data-opt-otimized-width="55" data-opt-optimized-height="55"><noscript><img
                                                decoding=async src="{{ asset('icons/Online-Icon-01-1.png') }}"
                                                data-hoverIn="{{ asset('icons/Online-Icon-03.png') }}"
                                                data-hoverOut="{{ asset('icons/Online-Icon-01-1.png') }}"
                                                alt="{{ $course->Course_Title }}" /></noscript></span>
                                </div>
                                <div class="contact-details-one__single-text">
                                    <h4><a href="/Course-Detail/{{ $course->Course_Title }}/{{ $course->id }}">{{ $course->Course_Title }}</a></h4>
                                    <ul>
                                        <li><strong>Date: </strong>{{ date('F d ', strtotime($course->Venue_Date))}} - {{ date('F d, Y', strtotime($course->Venue_Date. ' + '.$course->Course_Duration.'days' )) }}</li>
                                        <li><strong>Venue: </strong>{{ $course->Venue_Name }}</li>
                                        <li><strong>Price: </strong>$ {{ number_format(intval($course->Venue_Fee)) }}</li>
                                    </ul>
                                </div>
                            </div>
